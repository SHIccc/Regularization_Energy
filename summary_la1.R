...
SISoutput$ix%/%(90^2)+1
par(mgp=c(2, 0.5, 0))
plot(SISoutput$fit, main="(I)SIS-LASSO, lag=1", label = T)
plot(SISoutput$fit, main="(I)SIS-LASSO, lag=1", xvar = "dev", label = TRUE)
#cvfit1 =cv.glmnet(vecz, vecy)
load(file = "cvfit1.RData")
plot(cvfit1, ylim=c(0,1), main="(I)SIS-LASSO, lag=1")
...
la1.aic = log(abs(y1sig.avg)) + 2*11/54
la1.hq  = log(abs(y1sig.avg)) + 2*11*log(log(54))/54
la1.bic = log(abs(y1sig.avg)) + 2*11*log(54)/54
